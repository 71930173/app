...
const StudentIssueSelectionScreen = ({ navigation }) => {
  ...
  const load = useCallback(async () => {
    try { const res = await studentAPI.getIssueTypes(); setIssues(res.data || []); } catch (e) { console.error(e); }
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} />} contentContainerStyle={styles.scroll}>
        {issues.map((issue) => {
          ...
          return (
            <TouchableOpacity key={issue.id} style={[styles.card, { backgroundColor: theme.surface }]} onPress={() => navigation.navigate('StudentStaffSelection', { issueTypeId: issue.id, issueTypeName: name, issueTypeColor: color })}>
...
export default StudentIssueSelectionScreen;

...
const GuestNotificationsScreen = ({ navigation }) => {
  ...
  const load = useCallback(async () => {
    try { const res = await guestAPI.getNotifications(); setNotes(res.data || []); } catch (e) { console.error(e); }
  ...
  const markRead = async (id) => {
    try { await guestAPI.markNotificationRead(id); setNotes((prev) => prev.map((n) => n.id === id ? { ...n, is_read: 1 } : n)); } catch (e) { }
  };
...
const styles = StyleSheet.create({
  ...
  scroll: { padding: 16 }, card: { padding: 14, borderRadius: 12, marginBottom: 10 },
});

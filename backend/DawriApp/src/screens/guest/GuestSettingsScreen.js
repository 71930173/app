  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={{ color: theme.textSecondary, fontSize: 12, fontWeight: '600', textTransform: 'uppercase', marginBottom: 8, marginLeft: 4 }}>{t('language')}</Text>
        <SettingRow theme={theme} label={t('language')} value={lang === 'ar' ? t('arabic') : t('english')} onPress={toggleLanguage} />
        <Text style={{ color: theme.textSecondary, fontSize: 12, fontWeight: '600', textTransform: 'uppercase', marginBottom: 8, marginLeft: 4, marginTop: 16 }}>{t('theme')}</Text>
...
const SettingRow = ({ theme, label, value, onPress, isSwitch, value: switchVal, onValueChange }) => (
  <View style={[styles.row, { backgroundColor: theme.surface }]}>
    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14 }}>
      ...
      {isSwitch && <Switch value={switchVal} onValueChange={onValueChange} trackColor={{ false: theme.border, true: theme.primary }} thumbColor="#ffffff" />}
...
const styles = StyleSheet.create({
  ...
  scroll: { padding: 16 }, row: { borderRadius: 16, marginBottom: 8, overflow: 'hidden' },

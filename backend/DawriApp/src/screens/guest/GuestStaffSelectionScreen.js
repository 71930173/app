...
const GuestStaffSelectionScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    guestAPI.getAvailableStaff(issueTypeId).then((res) => setStaff(res.data || [])).catch(() => Toast.show({ type: 'error', text1: 'Error', text2: 'Failed to load staff' })).finally(() => setLoading(false));
  }, [issueTypeId]);
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await guestAPI.createAppointment({ staff_id: selected.id, issue_type_id: issueTypeId, description: desc });
      Toast.show({ type: 'success', text1: 'Success', text2: t('appointmentCreated') });
      navigation.navigate('GuestQueueStatus', { appointmentId: res.data.id });
    } catch (error) {
          ...
          <>
            {staff.map((s) => (
              <TouchableOpacity key={s.id} style={[styles.card, { backgroundColor: theme.surface, borderWidth: selected?.id === s.id ? 2 : 0, borderColor: theme.primary }]} onPress={() => setSelected(s)}>
                ...
                <View style={styles.cardBody}>
                  ...
                  <Text style={{ color: theme.textMuted, fontSize: 12 }}>{s.current_queue || 0} {t('waiting').toLowerCase()}</Text>
                </View>
            ...
            {selected && (
              <View style={{ marginTop: 8 }}>
                ...
                <TouchableOpacity style={[styles.btn, { backgroundColor: theme.primary }, submitting && { opacity: 0.6 }]} onPress={handleCreate} disabled={submitting}><Text style={styles.btnText}>{submitting ? t('loading') : t('confirmAppointment')}</Text></TouchableOpacity>
              </View>
            )}
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  scroll: { padding: 16 }, card: { flexDirection: 'row', alignItems: 'center', padding: 14, borderRadius: 14, marginBottom: 10 },
  avatar: { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center' },
  avatarText: { color: '#ffffff', fontSize: 16, fontWeight: 'bold' }, cardBody: { flex: 1, marginLeft: 12 },
  input: { borderWidth: 1, borderRadius: 12, padding: 12, fontSize: 15, minHeight: 80, textAlignVertical: 'top', marginBottom: 12 },
  btn: { paddingVertical: 14, borderRadius: 12, alignItems: 'center' }, btnText: { color: '#ffffff', fontSize: 16, fontWeight: '600' },
});

export default GuestStaffSelectionScreen;

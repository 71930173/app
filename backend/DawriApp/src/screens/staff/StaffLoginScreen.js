...
const StaffLoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    if (!validate()) return;
    setLoading(true);
    const result = await login({ email: email.trim(), password }, 'staff');
    setLoading(false);
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.header}>
          <View style={[styles.circle, { backgroundColor: theme.warning }]}><Text style={styles.circleText}>T</Text></View>
          <Text style={[styles.title, { color: theme.text }]}>{t('staffLogin')}</Text>
        </View>
        <InputField label={t('email')} placeholder="Enter your email" value={email} onChangeText={(v) => { setEmail(v); setErrors({ ...errors, email: null }); }} keyboardType="email-address" error={errors.email} />
        ...
        <TouchableOpacity style={[styles.btn, { backgroundColor: theme.warning }, loading && { opacity: 0.6 }]} onPress={handleLogin} disabled={loading}><Text style={styles.btnText}>{loading ? t('loading') : t('login')}</Text></TouchableOpacity>
      </ScrollView>
    </View>
  );
};
